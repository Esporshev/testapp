...
class Photos extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('source, fk', 'required'),
			array('fk', 'numerical', 'integerOnly'=>true),
			array('source', 'length', 'max'=>330),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, source, fk', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'fk0' => array(self::BELONGS_TO, 'Pages', 'fk'),
		);
	}
